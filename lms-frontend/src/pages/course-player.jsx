...
import { useFetchUserEnrollmentsQuery, useMarkLectureCompleteMutation } from "@/store/services/enrollment-service";
...
export default function CoursePlayerPage() {
  ...
  const completedLectureIds = useMemo(
    () =>
      new Set(
        (enrollment?.completedLectures || []).map((lecture) => (typeof lecture === "object" ? lecture?._id : lecture)),
      ),
    [enrollment],
  );
  ...
  async function summarizeLecture() {
    ...
    try {
      const res = await fetch(backendURL + `/api/lectures/${currentLecture._id}/summarize`, {
        method: "POST",
      });
  ...
  if (isLoading) {
    return (
      <main className="grid gap-4 lg:grid-cols-[1fr_280px]" aria-busy="true" aria-label="Loading course player">
  ...
  if (error) {
    return (
      <main>
        <p className="text-sm text-destructive">{getApiErrorMessage(error, "Unable to fetch lectures.")}</p>
      </main>
    );
  }
  ...
  return (
    <main className="grid gap-4 lg:grid-cols-[1fr_280px]">
      <section aria-labelledby="current-lecture-title">
        <Card>
          <CardHeader>
            <CardTitle id="current-lecture-title">{currentLecture?.title || "Course player"}</CardTitle>
          ...
          <CardContent className="grid gap-4">
            ...
            {enrollment ? (
              <>
                <section className="grid gap-3" aria-label="Enrollment progress">
                ...
                {(summary || isSummarizing) && (
                  <section className="grid gap-2 border-t pt-4" aria-label="Lecture summary">
                    ...
                    >
                      ...
                      {isSummarizing && displayedSummary === summary && <span className="animate-pulse">|</span>}
                    </div>
                  </section>
                )}
              </>
            ) : (
              <section className="flex flex-wrap items-center gap-3" aria-label="Enrollment required">
          ...
          <CardFooter className="flex flex-col items-start gap-4">
            <section className="w-full border-t pt-4 grid gap-4" aria-label="Lecture transcript">
              ...
              <ChatSheet lecture={currentLecture} />
            </section>
