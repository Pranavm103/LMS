...
export async function askLectureQuestion(req, res) {
    try {
        const { answer, error, status } = await answerLectureQuestion(req.params.id, req.body?.question);
        ...
        res.status(500).json({ message: "Unable to answer question", error: error.message });
    }
}
...
export async function addLectureToCourse(req, res) {
    try {
        ...
        const { lecture, error, status } = await createLecture(req.user, courseId, req.body);
        ...
        res.status(400).json({ message: "Unable to add lecture", error: ex.message });
    }
}

export async function updateLecture(req, res) {
    try {
        const { lecture, error, status } = await editLecture(req.user, req.params.id, req.body);
        ...
        res.status(400).json({ message: "Unable to update lecture", error: ex.message });
    }
}

export async function deleteLecture(req, res) {
    try {
        const { message, error, status } = await removeLecture(req.user, req.params.id);
        ...
        res.status(500).json({ message: "Unable to delete lecture", error: ex.message });
    }
}
