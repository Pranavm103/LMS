...
export async function createNewCourse(user, courseData) {
    if (!isConnected()) {
        return { status: 503, error: "Database is not connected. Please start MongoDB and try again." };
    }
    ...
    const thumbnail = courseData.thumbnail?.trim();
    ...
    if (!thumbnail) {
        return { status: 400, error: "Thumbnail is required" };
    }
